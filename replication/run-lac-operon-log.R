...
for(j in 1:(ncol(xsim)-1)){
  xsim[,1+j] <- xsim[,1+j]*exp(rnorm(nrow(xsim), sd=config$noise[j]))
}
...
xsim.obs[,-1] <- log(xsim.obs[,-1])
matplot(xsim.obs$time, xsim.obs[,-1], type="p", col=1:(ncol(xsim)-1), pch=20)

xsim <- setDiscretization(xsim.obs,config$filllevel)

dynamicalModelList <- list(
  fOde=magi:::lacOperonLogODE,
  fOdeDx=magi:::lacOperonLogDx,
  fOdeDtheta=magi:::lacOperonLogDtheta,
...
testDynamicalModel(dynamicalModelList$fOde, dynamicalModelList$fOdeDx, dynamicalModelList$fOdeDtheta, "dynamicalModelList", xInitExogenous[1:50,], pram.true$theta, xsim$time)
