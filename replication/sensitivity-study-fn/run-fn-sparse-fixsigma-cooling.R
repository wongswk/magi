...
outDir <- paste0("../results/fn-sparse-fixsigma-cooling-fill", filllevel, "-nobs", nobs_keep, "/")
...
thetaInitCooling <- colMeans(gpode$theta)
xInitCooling <- apply(gpode$xsampled, 2:3, mean)
sigmaInitCooling <- apply(gpode$sigma, 2, mean)
...
samplesCpp <- magi:::solveMagiRcpp(
  ...
  odeModel = fnmodel,
  tvecFull = xsim$time,
  sigmaExogenous = sigmaInitCooling,
  phiExogenous = phiUsed,
  xInitExogenous = xInitCooling,
  thetaInitExogenous = thetaInitCooling,
  muExogenous = matrix(nrow=0,ncol=0),
  dotmuExogenous = matrix(nrow=0,ncol=0),
  priorTemperatureLevel = config$priorTemperature,
  priorTemperatureDeriv = config$priorTemperature,
