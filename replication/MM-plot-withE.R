...
pdf(width = 15, height = 5, file=paste0(filename, " - present.pdf"))
# layout(rbind(c(1,2,3,4), c(5,5,5,5)), heights = c(5,1))
layout(rbind(c(1,2,3)), heights = c(5,1))

matplot(xtrue[, "time"], (xtrue[, -1]), type="l", lty=1, col=c(1:6), xlab="time", ylab=NA)
matplot(xsim.obs$time, (xsim.obs[,-1]), type="p", col=c(1:6), pch=20, add = TRUE)
mtext('sample observations', cex=1.5)
title("Michaelis Menten")
legend("right", c("true unobserved [E]", "true [S]", "true [P]", "observed [S]", "observed [P]"),
       lty=c(1,1,1,NA,NA), pch=c(NA,NA,NA,20,20), col=c(1:3, 2:3), cex=1.5)
...
for (i in 2:3) {
  ...
  mtext(compnames[i])
  if(i == 2){
    legend("topright", c("truth", "median of all inferred trajectories", "95% interval from the 2.5 and 97.5 percentile\nof all inferred trajectories"), lty=c(1,1,0), lwd=c(4,3,0),
           col = c("red", "forestgreen", NA), fill=c(0, 0,"skyblue"), #bty = "n", #text.width=c(0, 0.4, 0.05),
           border=c(0, 0, "skyblue"), pch=c(NA, NA, 15), cex=1.2)
  }else{
    legend("bottomright", c("truth", "median of all inferred trajectories", "95% interval from the 2.5 and 97.5 percentile\nof all inferred trajectories"), lty=c(1,1,0), lwd=c(4,3,0),
           col = c("red", "forestgreen", NA), fill=c(0, 0,"skyblue"), #bty = "n", #text.width=c(0, 0.4, 0.05),
           border=c(0, 0, "skyblue"), pch=c(NA, NA, 15), cex=1.2)
...
#        col = c("red", "forestgreen", NA), fill=c(0, 0,"skyblue"), bty = "n", #text.width=c(0, 0.4, 0.05),
#        border=c(0, 0, "skyblue"), pch=c(NA, NA, 15), cex=0.8, horiz=TRUE)
dev.off()


pdf(width = 6, height = 3.1, file=paste0(filename, " - param hist.pdf"))
...
pdf(width = 20, height = 5, file=paste0(filename, " - present with E.pdf"))
layout(rbind(c(1,2,3,4), c(5,5,5,5)), heights = c(5,1))
# layout(rbind(c(1,2,3)), heights = c(5,1))
...
matplot(xtrue[, "time"], (xtrue[, -1]), type="l", lty=1, col=c("grey50",6,4), xlab="time", ylab=NA)
matplot(xsim.obs$time, (xsim.obs[,-1]), type="p", col=c("grey50",6,4), pch=20, add = TRUE)
mtext('sample observations', cex=2)

legend("right", c("true [P]", "true [S]", "true [E]", "observed [P]", "observed [S]"), 
       lty=c(1,1,1,NA,NA), pch=c(NA,NA,NA,20,20), col=c(4,6,"grey50"), cex=1.8)
...
for (i in 3:1) {
  ...
  lines(times, ourEst, col="forestgreen", lwd=3)
  if (i == 1){
    mtext(expression(paste("[E] ", bold("(Unobserved)"))), cex=2)  
  }else{
    mtext(paste(compnames[i]), cex=2)  
  }
  
  if (i==3){
    zoomin_x <- c(200, 280)
    zoomin_y <- c(0.48, 0.66)
    
    zoomout_x <- c(350, 1100)
    zoomout_y <- c(0.1, 0.6)
  }else if(i==2){
    zoomin_x <- c(200, 280)
    zoomin_y <- c(0.32, 0.51)
    ...
    next
    # zoomin_x <- c(500, 660)
    # zoomin_y <- c(0.093, 0.1)
    # 
    # zoomout_x <- c(150, 1100)
    # zoomout_y <- c(0.0, 0.07)
  }
...
legend("center", c("truth", "median of all inferred trajectories", "95% interval from the 2.5 and 97.5 percentile of all inferred trajectories"), lty=c(1,1,0), lwd=c(4,3,0),
       col = c("red", "forestgreen", NA), fill=c(0, 0,"skyblue"), text.width=c(0.0, 0.55, 0.05), bty = "n",
       border=c(0, 0, "skyblue"), pch=c(NA, NA, 15), cex=2.3, horiz=TRUE)
