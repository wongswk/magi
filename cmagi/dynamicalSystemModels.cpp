...
// [[Rcpp::export]]
arma::mat MichaelisMentenInhibitor6ODE(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
  const double e0 = 0.1;
  const vec & e = x.col(0);
  const vec & s = x.col(1);
  const vec & p = x.col(2);
  ...
  resultdt.col(3) = -theta[3] * i % e + theta[4] * ei;
  resultdt.col(4) = theta[3] * i % e - theta[4] * ei;
  resultdt.col(5) = theta[0] * e % s  - (theta[1]+theta[2]) * es;
...
// [[Rcpp::export]]
arma::cube MichaelisMentenInhibitor6Dx(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
  cube resultDx(x.n_rows, x.n_cols, x.n_cols, fill::zeros);
  ...
  resultDx.slice(0).col(0) = -theta[0] * s - theta[3] * i;
  resultDx.slice(0).col(1) = -theta[0] * e;
  resultDx.slice(0).col(3) = -theta[3] * e;
  resultDx.slice(0).col(4).fill(theta[4]);
  resultDx.slice(0).col(5).fill(theta[1]+theta[2]);
  
  resultDx.slice(1).col(0) = -theta[0] * s;
  resultDx.slice(1).col(1) = -theta[0] * e;
  resultDx.slice(1).col(5).fill(theta[1]);
  
  resultDx.slice(2).col(5).fill(theta[2]);
  ...
  resultDx.slice(5).col(0) = theta[0] * s;
  resultDx.slice(5).col(1) = theta[0] * e;
  resultDx.slice(5).col(5).fill(-(theta[1]+theta[2]));
...
// [[Rcpp::export]]
arma::cube MichaelisMentenInhibitor6Dtheta(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
  cube resultDtheta(x.n_rows, theta.size(), x.n_cols, fill::zeros);
  ...
  resultDtheta.slice(5).col(0) = e % s;
  resultDtheta.slice(5).col(1) = -es;
  resultDtheta.slice(5).col(2) = -es;
...
arma::mat MichaelisMentenlog1xModelODE(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    ...
    const vec & e = arma::exp(x.col(0)) - delta;
    const vec & s = arma::exp(x.col(1)) - delta;
    const vec & es = arma::exp(x.col(2)) - delta;
    const vec & p = arma::exp(x.col(3)) - delta;

    mat resultdt(x.n_rows, x.n_cols);

    resultdt.col(0) = (-theta[0] * e % s + (theta[1]+theta[2]) * es) / (e + delta);
    resultdt.col(1) = (-theta[0] * e % s + (theta[1]) * es) / (s + delta);
    resultdt.col(2) = (theta[0] * e % s - (theta[1]+theta[2]) * es) / (es + delta);
    resultdt.col(3) = (theta[2] * es) / (p + delta);
...
arma::cube MichaelisMentenlog1xModelDx(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    ...
    resultDx.slice(0).col(0) = -theta[0] * s - (-theta[0] * e % s + (theta[1]+theta[2]) * es) / (e + delta);
    resultDx.slice(0).col(1) = -theta[0] * e % (s + delta) / (e + delta);
    resultDx.slice(0).col(2) = (theta[1] + theta[2]) * (es + delta) / (e + delta);

    resultDx.slice(1).col(0) = -theta[0] * s / (s + delta) % (e + delta);
    resultDx.slice(1).col(1) = -theta[0] * e - (-theta[0] * e % s + (theta[1]) * es) / (s + delta);
    resultDx.slice(1).col(2) = theta[1] / (s + delta) % (es + delta);

    resultDx.slice(2).col(0) = theta[0] * s / (es + delta) % (e + delta);
    resultDx.slice(2).col(1) = theta[0] * e / (es + delta) % (s + delta);
    resultDx.slice(2).col(2) = (-theta[1] - theta[2]) - (theta[0] * e % s - (theta[1]+theta[2]) * es) / (es + delta);

    resultDx.slice(3).col(2) = theta[2] / (p + delta) % (es + delta);
    resultDx.slice(3).col(3) = -(theta[2] * es) / (p + delta);
...
arma::cube MichaelisMentenlog1xModelDtheta(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    ...
    resultDtheta.slice(0).col(0) = (-e % s) / (e + delta);
    resultDtheta.slice(0).col(1) = es / (e + delta);
    resultDtheta.slice(0).col(2) = es / (e + delta);

    resultDtheta.slice(1).col(0) = (-e % s) / (s + delta);
    resultDtheta.slice(1).col(1) = es / (s + delta);

    resultDtheta.slice(2).col(0) = e % s / (es + delta);
    resultDtheta.slice(2).col(1) = -es / (es + delta);
    resultDtheta.slice(2).col(2) = -es / (es + delta);

    resultDtheta.slice(3).col(2) = es / (p + delta);
...
arma::cube MichaelisMentenModelVb6pDx(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    ...
    resultDx.slice(1).col(3).fill(km2);
...
// [[Rcpp::export]]
arma::mat lacOperonLogODE(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    const double delta = 1e-4;

    const arma::mat expx = arma::exp(x);

    const vec & ri = expx.col(0) - delta;
    const vec & i = expx.col(1) - delta;
    const vec & lactose = expx.col(2) - delta;
    const vec & ilactose = expx.col(3) - delta;
    const vec & op = expx.col(4) - delta;
    const vec & iop = expx.col(5) - delta;
    const vec & rnap = expx.col(6) - delta;
    const vec & rnapo = expx.col(7) - delta;
    const vec & r = expx.col(8) - delta;
    const vec & z = expx.col(9) - delta;
...
// [[Rcpp::export]]
arma::cube lacOperonLogDx(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    const double delta = 1e-4;
...
//    const double iconstant = theta(0);
    ...
    const arma::mat fode = lacOperonLogODE(theta, x, tvec);

    for (unsigned i1 = 0; i1 < 10; i1 ++){
        for (unsigned i2 = 0; i2 < 10; i2 ++){
            if(i1 != i2){
                resultDx.slice(i1).col(i2) = resultDx.slice(i1).col(i2) % expx.col(i2) / expx.col(i1);
            }else{
                resultDx.slice(i1).col(i2) = -fode.col(i1) + resultDx.slice(i1).col(i2);
            }
        }
    }
...
// [[Rcpp::export]]
arma::cube lacOperonLogDtheta(const arma::vec & theta, const arma::mat & x, const arma::vec & tvec) {
    const double delta = 1e-4;
...
//    resultDtheta.slice(0).col(0).fill(k(1));
    ...
    for (unsigned i1 = 0; i1 < 10; i1 ++){
        for (unsigned i2 = 0; i2 < 17; i2 ++){
            resultDtheta.slice(i1).col(i2) /= expx.col(i1);
        }
    }
