...
Use a numerical solver to generate the true trajectories:
```{r}
...
saveRDS(gpode, "examples/represillator.rds")
```
...
```
```{r, echo=FALSE, eval=TRUE}
saveRDS(gpode, "examples/M-M.rds")
```
```{r, echo=FALSE, eval=FALSE}
gpode <- readRDS("examples/M-M.rds")
```
...
```
```{r, echo=FALSE, eval=TRUE}
saveRDS(gpode, "examples/lac-operon.rds")
```
```{r, echo=FALSE, eval=FALSE}
gpode <- readRDS("examples/lac-operon.rds")
```
...
for (i in 1:10) {
  ...
  plot(c(min(tvecsolve), max(tvecsolve)),
       c(min(c(recon[,i+1], xtrue[xtrue$time >=1,i+1], xsim.obs[,i+1])),
         max(c(recon[,i+1], xtrue[xtrue$time>=1,i+1], xsim.obs[,i+1]))),
       type = 'n', ylab='', xlab='')
  mtext(compnames[i])
...
Divide into train/test to create simulated noisy data:
```{r}
...
Run the MAGI method under the correct inhibitor model:
```{r, results='hide', eval=TRUE}
dynamicalModelList <- list(
...
Run the MAGI method under the incorrect vanilla M-M model:
```{r, results='hide', eval=TRUE}
