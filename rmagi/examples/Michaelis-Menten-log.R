...
outDir <- "../results/Michaelis-Menten-Log/"
...
# if(!exists("config")){
  config <- list(
    nobs = nrow(realdata),
    noise = c(NA, 0.01, NA, 0.01),  # noise = c(0.01, 0.01, 0.01, 0.01), for fully observed case
    ...
    linfillspace = 0.5, 
    t.end = 70,
    modelName = "Michaelis-Menten-Log"
  )
# }
...
modelODE <- function(t, state, parameters) {
  list(as.vector(magi:::MichaelisMentenLogModelODE(parameters, t(state), t)))
}
...
dynamicalModelList <- list(
  fOde=magi:::MichaelisMentenLogModelODE,
  fOdeDx=magi:::MichaelisMentenLogModelDx,
  fOdeDtheta=magi:::MichaelisMentenLogModelDtheta,
  thetaLowerBound=c(0,0,0),
  thetaUpperBound=c(Inf,Inf,Inf),
  name="Michaelis-Menten-Log"
)
