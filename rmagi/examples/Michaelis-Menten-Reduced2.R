...
modelODE <- function(t, state, parameters) {
  list(as.vector(magi:::MichaelisMentenReduced2ODE(parameters, t(state), t)))
}

xtrue <- deSolve::ode(y = pram.true$x0, times = times, func = modelODE, parms = pram.true$theta)
xtrue <- data.frame(xtrue)
if(is.null(config$skip_visualization)){
  matplot(xtrue[, "time"], xtrue[, c(2,3)], type="l", lty=1)  
}
...
dynamicalModelList <- list(
  fOde=magi:::MichaelisMentenReduced2ODE,
  fOdeDx=magi:::MichaelisMentenReduced2Dx,
  fOdeDtheta=magi:::MichaelisMentenReduced2Dtheta,
  thetaLowerBound=c(0,-100,0),
  thetaUpperBound=c(Inf,Inf,Inf),
  name="Michaelis-Menten-Reduced2"
)
