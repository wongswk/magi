...
// Get double real matrix as vector from Matlab/Octave.  ** Added SW **
inline
Col<double>
armaGetPrVec(const mxArray *matlabMatrix, bool copy_aux_mem = false, bool strict = true)
  {
  if(mxGetData(matlabMatrix) != NULL)
    {
    const mwSize n_dim = mxGetNumberOfDimensions(matlabMatrix);
    ...
    if(n_dim == 2)
      {
      return Col<double>(mxGetPr(matlabMatrix), mxGetN(matlabMatrix), copy_aux_mem, strict);
      }
    else
      {
      mexErrMsgTxt("Number of dimensions must be 2.");
