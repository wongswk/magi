...
if config.useExoSigma
    exoSigma = 0.001 * ones(1,5);  %config.noise;
else
    exoSigma = [];
end
...
fillC = 0:config.linfillspace:100;
...
ptmodel.thetaUpperBound= [100 100 100 100 100 100];
...
outFileName =  sprintf('result-%s-noise%.3f-linspace%.2f-useExoSigma%d-useFixedSigma%d-linearizeInit%d-useBand%d-bandsize%d-numEpoch%d', config.modelName, config.noise(1), config.linfillspace, config.useExoSigma, config.useFixedSigma, config.linearizexInit, config.useBand, config.bandsize, config.max_epoch);
set(groot, 'defaultFigureUnits','normalized')
set(groot, 'defaultFigurePosition',[0 0 1 1])


for epoch=1:config.max_epoch
    
    outCpp = samplesCpp(:,:,epoch);
    outCpp(:,1) = [];
    lliklist = outCpp(1,:);
    ...
    [maxllik, MAPIndex] = max(lliklist( ((config.n_iter-1) * config.burninRatio):(config.n_iter-1)));
    MAPIndex = MAPIndex + (config.n_iter-1) * config.burninRatio - 1;
