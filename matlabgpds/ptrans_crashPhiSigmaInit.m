...
config.nobs = 15;
config.noise = 0.001 * ones(1,5); % 0.01 high noise, 0.001 low noise
config.t_end = 100;
...
if config.linearizexInit
    xsim = xsim(:,1)';
    xsim = horzcat(xsim',interp1(xsim_obs(:,1),xsim_obs(:,2),xsim)',interp1(xsim_obs(:,1),xsim_obs(:,3),xsim)',interp1(xsim_obs(:,1),xsim_obs(:,4),xsim)',interp1(xsim_obs(:,1),xsim_obs(:,5),xsim)',interp1(xsim_obs(:,1),xsim_obs(:,6),xsim)');
    exoxInit = xsim(:,2:size(xsim,2));
else
    exoxInit = [];
end
...
outFileName =  sprintf('result-%s-noise%.3f-fill%d-useExoSigma%d-linearizeInit%d', config.modelName, config.noise(1), config.filllevel, config.useExoSigma, config.linearizexInit);
