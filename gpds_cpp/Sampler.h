...
class Sampler {
    const arma::mat & yobs;
    const vector<gpcov> & covAllDimensions;
    const int nsteps;
    const bool traj;
...
public:
    ...
    void sampleChian(const arma::vec &xthetasigmaInit, const arma::vec &stepLowInit);
    Sampler(const arma::mat & yobsInput,
            const vector<gpcov> & covAllDimensionsInput,
...
void Sampler::sampleChian(const arma::vec &xthetasigmaInit, const arma::vec &stepLowInit, bool verbose=false) {
    ...
    const unsigned int numparam = yobsInput.size() + modelInput.thetaSize + yobsInput.n_cols;
    ...
    for (unsigned int t = 1; t < niter; t++){
        ...
        double acceptRate = accepts(arma::span(std::max(0, t - 99), t)).mean();
        if (t < burnin && t > 10){
            if (acceptRate > 0.9){
                stepLow *= 1.005;
