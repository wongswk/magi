...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$kernel,"-",config$seed,"-",date(),"-hes1-async-partial-observed-trueSigma-phase2.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, configWithPhiSig, odemodel)
...
saveRDS(absCI, paste0(
  outDir,
  config$loglikflag,"-hes1-async-partial-observed-trueSigma-phase2-",config$kernel,"-",config$seed,".rds"))
save.image(paste0(
  outDir,
  config$loglikflag,"-hes1-async-partial-observed-trueSigma-phase2-",config$kernel,"-",config$seed,".rda"))

pdf(paste0(outDir, config$kernel,"-",config$seed,"-",date(),"-hes1-async-partial-observed-trueSigma-phase2-addon.pdf"), 
    width = 8, height = 8)
layout(matrix(1:4, 2))
matplot(xtrue$time, xtrue[,-1], type="l", lty=1, main="raw data")
matplot(xsim$time, xsim[,-1], type="p", pch=20, add=TRUE)
...
for(epoch in epoch_sequence){
  ...
  gpds:::plotPostSamplesFlex(
    paste0(outDir, config$kernel,"-",config$seed,"-",date(),"-hes1-async-partial-observed-trueSigma-phase",epoch,".pdf"), 
    xtrue, dotxtrue, xsim, gpode, pram.true, configWithPhiSig, odemodel)
  ...
  saveRDS(absCI, paste0(
    outDir,
    config$loglikflag,"-hes1-async-partial-observed-trueSigma-phase",epoch,"-",config$kernel,"-",config$seed,".rds"))
  save.image(paste0(
    outDir,
    config$loglikflag,"-hes1-async-partial-observed-trueSigma-phase",epoch,"-",config$kernel,"-",config$seed,".rda"))
