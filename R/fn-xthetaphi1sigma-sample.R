...
nophi1Cov <- lapply(1:(ncol(xsim.obs)-1), function(j){
  covEach <- calCov(c(1, curphi[2, j]), r, signr, bandsize=config$bandsize, 
...
if(config$useGPmean){
  for(j in 1:(ncol(xsim)-1)){
    ...
    nophi1Cov[[j]]$mu <- as.vector(ydy[[1]])
    nophi1Cov[[j]]$dotmu <- as.vector(ydy[[2]])
  }
}
...
thetaoptim <- function(xInit, thetaInit, curphi, cursigma){
  fullphiCov <- lapply(1:(ncol(xsim.obs)-1), function(j){
  ...
  fn <- function(par) {
    -xthetallikRcpp( yobs, fullphiCov, cursigma, c(xInit, par), "FN" )$value
  }
  gr <- function(par) {
    -as.vector(xthetallikRcpp( yobs, fullphiCov, cursigma, c(xInit, par), "FN" )$grad[-(1:length(xInit))])
  }
...
phi1Init <- curphi[1,]
xthetaphi1sigmaInit <- c(xInit, thetaInit, phi1Init, sigmaInit)
stepLowInit <- rep(0.000035, length(xthetaphi1sigmaInit))
...
phi1Id <- (max(thetaId)+1):(max(thetaId)+length(phi1Init))
sigmaId <- (max(phi1Id)+1):(max(phi1Id)+length(sigmaInit))

xthetaphi1sigamSingleSampler <- function(xthetaphi1sigma, stepSize) 
  xthetaphi1sigmaSample(yobs, nophi1Cov, xthetaphi1sigma[phi1Id], xthetaphi1sigma[sigmaId], xthetaphi1sigma[c(xId, thetaId)], 
...
chainSamplesOut <- chainSampler(config, xthetaphi1sigmaInit, xthetaphi1sigamSingleSampler, stepLowInit, verbose=TRUE)
...
gpode <- list(theta=chainSamplesOut$xth[-(1:burnin), thetaId],
              ...
              phi1sampled = chainSamplesOut$xth[-(1:burnin), phi1Id],
              phi = curphi)
gpode$fode <- sapply(1:length(gpode$lglik), function(t) 
  with(gpode, gpds:::fnmodelODE(theta[t,], xsampled[t,,])), simplify = "array")
gpode$fode <- aperm(gpode$fode, c(3,1,2))

matplot(gpode$phi1sampled, type="l", main="phi1")
...
odemodel <- list(times=times, modelODE=modelODE, xtrue=xtrue, curCov=nophi1Cov)
...
if(config$phase2){
  ...
  for(j in 1:ncol(muAllDim)){
    nophi1Cov[[j]]$mu <- muAllDim[,j]
    nophi1Cov[[j]]$dotmu <- dotmuAllDim[,j]
  }
