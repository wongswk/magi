...
all_files <- lapply(all_files, function(x) x[grep(".*log-([0-9]+)-fullobs.*", x)])
...
dir.create("../results/cpp/fullobs/ramsay", showWarnings = FALSE)
...
print(each_seed)
load(paste0("../results/cpp/fullobs/variablephi-notemper/Hes1-log-",each_seed,"-fullobs-variablephi-notemper.rda"), envir = envhes1log)
...
save(list=setdiff(ls(), "envhes1log"), file = paste0("../results/cpp/fullobs/ramsay/Hes1-log-",each_seed,"-fullobs-ramsay.rda"))
dev.off()
