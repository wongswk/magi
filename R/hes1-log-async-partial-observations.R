...
for(epoch in epoch_sequence){
  ...
  gpds:::plotPostSamplesFlex(
    paste0(outDir, config$kernel,"-",config$seed,"-",date(),"-hes1-log-async-partial-observed-trueSigma-phase",epoch,".pdf"), 
    xtrue, dotxtrue, xsim, gpode, pram.true, configWithPhiSig, odemodel)
  ...
  saveRDS(absCI, paste0(
    outDir,
    config$loglikflag,"-hes1-log-async-partial-observed-trueSigma-phase",epoch,"-",config$kernel,"-",config$seed,".rds"))
  save.image(paste0(
    outDir,
    config$loglikflag,"-hes1-log-async-partial-observed-trueSigma-phase",epoch,"-",config$kernel,"-",config$seed,".rda"))
}
