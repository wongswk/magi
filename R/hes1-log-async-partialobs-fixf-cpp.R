...
pram.true <- list(
  theta = c(0.022, 0.3, 0.031, 0.028, 0.5, 0.3),
...
modelODE <- function(t, state, parameters) {
  list(as.vector(gpds:::hes1logmodelODEfixf(parameters, t(state))))
}
...
gpode$fode <- sapply(1:length(gpode$lglik), function(t) 
  with(gpode, gpds:::hes1logmodelODEfixf(theta[t,], xsampled[t,,])), simplify = "array")
gpode$fode <- aperm(gpode$fode, c(3,1,2))

dotxtrue = gpds:::hes1logmodelODEfixf(pram.true$theta, data.matrix(xtrue[,-1]))
...
save.image(paste0(outDir, config$modelName,"-",config$seed,"-fixf.rda"))
