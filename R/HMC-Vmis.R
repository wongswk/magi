...
th.all[1,] <-  c( rep(0,41), apply(gpfit_ss$rtrue, 2, mean), 1, 1, 1)
phisig[1,] <- c(1, 1,  apply(gpfit_ss$rphi, 2, mean), mean(gpfit_ss$sigma))
...
loglikVmis( VRtrue[seq(1,401,length=41),], c(0.2,0.2,3), bestCovV, bestCovR, 0.1,  fn.sim[,1:2])

## loglik at degenerate case (zero curve)
loglikVmis(matrix(0,nrow=41,ncol=2),c(0,1,1),calCov(c(.1,10)), calCov(c(.1,10)), 0.25, fn.sim[,1:2])
...
cursigma <- phisig[1,5]
curllik <- xthUVmis(th.all[1,])

full_llik <- c()
lliklist <- c()
lliklist[1] <- curllik
full_llik[1] <- loglikVmis( cbind(th.all[1,1:41],th.all[1,42:82]), th.all[1,83:85], curCovV, curCovR, cursigma,  fn.sim[,1:2])
accepts <- 0
paccepts <- 0
#deltas <- c()
...
for (t in 2:n.iter) {
  ...
  # Update X and theta
  foo <- basic_hmc(xthUVmis, step=runif(1,0.0002,0.0005), nsteps= 20, initial=th.all[t-1,], return.traj = T)
  lliklist[t] <- foo$lpr
  th.all[t,] <- foo$final
  #deltas[t] <- foo$delta
  ...
  oldCovV <- curCovV
  oldCovR <- curCovR
  old_ll <- loglikVmis( cbind(th.all[t,1:41], th.all[t,42:82]), th.all[t,83:85], oldCovV, oldCovR, phisig[t-1,5], fn.sim[,1:2])
  ...
  if( min(ps_prop - lower_b) > 0 && min(upper_b - ps_prop) > 0) {  # check bounds
    ...
    prop_ll <- loglikVmis( cbind(th.all[t,1:41], th.all[t,42:82]), th.all[t,83:85], propCovV, propCovR, ps_prop[5], fn.sim[,1:2])
  } else {
  ...
  full_llik[t] <- loglikVmis( cbind(th.all[t,1:41],th.all[t,42:82]), th.all[t,83:85], curCovV, curCovR, cursigma,  fn.sim[,1:2])  
}  

## Best sampled
id.best <- which.max(full_llik)
loglikVmis( cbind(th.all[id.best,1:41],th.all[id.best,42:82]), th.all[id.best,83:85], calCov(phisig[id.best,1:2]),calCov(phisig[id.best,3:4]), phisig[id.best,5],  fn.sim[,1:2])

pdf(file="R-HMC-output-Vmis.pdf")
par(mfrow=c(2,2))
