...
thetaoptim <- function(xInit, thetaInit, curphi, cursigma){
  ...
  fn <- function(par) {
    -xthetallikRcpp( yobs, curCov, cursigma, c(xInit, par), "HIV" )$value
  }
  gr <- function(par) {
    -as.vector(xthetallikRcpp( yobs, curCov, cursigma, c(xInit, par), "HIV" )$grad[-(1:length(xInit))])
  }
  marlikmap <- optim(c(thetaInit), fn, gr, 
                     method="L-BFGS-B", lower = -Inf, control = list(maxit=1e5))
...
xthetaoptim <- function(xInit, thetaInit, curphi, cursigma, priorTemperature = rep(1,2)){
  ...
  fn <- function(par) {
    -xthetallikRcpp( yobs, curCov, cursigma, par, "HIV", priorTemperatureInput=priorTemperature )$value
  }
  gr <- function(par) {
    -as.vector(xthetallikRcpp( yobs, curCov, cursigma, par, "HIV", priorTemperatureInput=priorTemperature )$grad)
  }
...
xthetasigmaoptim <- function(xInit, thetaInit, curphi, sigmaInit, priorTemperature = rep(1,2)){
  ...
  fn <- function(par) {
    ...
    -xthetasigmallikRcpp( xthis, thetathis, sigmathis, yobs, curCov, priorTemperature, FALSE, "HIV" )$value
  }
  gr <- function(par) {
    xthis <- matrix(par[xId], ncol=ncol(yobs))
    thetathis <- par[thetaId]
    sigmathis <- par[(max(thetaId)+1):length(par)]
    -as.vector(xthetasigmallikRcpp( xthis, thetathis, sigmathis, yobs, curCov, priorTemperature, FALSE, "HIV" )$grad)
  }
