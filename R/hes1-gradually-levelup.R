...
stepLowXInit <- head(chainSamplesOut$stepLow, length(xInit))
stepLowXInit <- matrix(stepLowXInit, ncol=3)
xInitFill <- (xInit[-1, ] + xInit[-nrow(xInit), ])/2
stepLowXInitFill <- (stepLowXInit[-1, ] + stepLowXInit[-nrow(stepLowXInit), ])/2
xInit <- data.matrix(insertNaN(data.frame(time=1:nrow(xInit), xInit), 1)[,-1])
stepLowXInit <- data.matrix(insertNaN(data.frame(time=1:nrow(stepLowXInit), stepLowXInit), 1)[,-1])
xInit[is.na(xInit)] <- xInitFill
stepLowXInit[is.na(stepLowXInit)] <- stepLowXInitFill
thetaInit <- colMeans(gpode$theta)
stepLowInit <- c(stepLowXInit, tail(chainSamplesOut$stepLow, length(thetaInit)))
...
cursigma <- pram.true$sigma
x3thetaphi3Init <- x3thetaphi3sgd(xInit, thetaInit, curphi, cursigma, maxit=100, learningRate=1e-8)
xInit <- x3thetaphi3Init$xInit
curphi <- x3thetaphi3Init$curphi
thetaInit <- x3thetaphi3Init$thetaInit
