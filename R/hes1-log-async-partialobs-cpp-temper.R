...
save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-coldstart.rda"))
...
## tempered with warm start and update missing component phi from warm start ----------------------------
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatemissingphi.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatemissingphi.rda"))

## tempered with warm start and re-optimize phi from warm start ----------------------------
...
outDir <- "../results/cpp/7param/variablephi-temper-warmstart-reoptimizephi/"
system(paste("mkdir -p", outDir))
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-reoptimizephi.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-reoptimizephi.rda"))


## tempered with warm start and update phi from warm start posterior draws (not mean) ----------------------------
rm(list=setdiff(ls(), "config"))
load(paste0("../results/cpp/7param/variablephi-notemper/", config$modelName,"-",config$seed,"-7param-variablephi-notemper.rda"))
...
phiNewInitSamples <- parallel::mclapply(seq(1, dim(gpode$xsampled)[1], length=1000), function(i){
  xInit <- gpode$xsampled[i,,]
  phiNewInit <- gpds:::solveGpdsRcpp(
    yFull = xInit,
    odeModel = hes1logmodel,
    tvecFull = xsim$time,
...
phiNewInitSamples <- sapply(phiNewInitSamples, identity, simplify = "array")
phiNewMedian <- apply(phiNewInitSamples, 1:2, median)
...
outDir <- "../results/cpp/7param/variablephi-temper-warmstart-updatephi-posterior-sample/"
system(paste("mkdir -p", outDir))
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatephi-posterior-sample.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatephi-posterior-sample.rda"))

## tempered with warm start and re-optimize phi from warm start posterior draws (not mean) ----------------------------
rm(list=setdiff(ls(), "config"))
load(paste0("../results/cpp/7param/variablephi-notemper/", config$modelName,"-",config$seed,"-7param-variablephi-notemper.rda"))
...
phiNewInitSamples <- parallel::mclapply(seq(1, dim(gpode$xsampled)[1], length=100), function(i){
  xInit <- gpode$xsampled[i,,]
  thetaInit <- gpode$theta[i,]
  phiNewInit <- gpds:::solveGpdsRcpp(
    yFull = data.matrix(xsim[,-1]),
    odeModel = hes1logmodel,
    tvecFull = xsim$time,
...
## tempered with warm start and update phi from warm start posterior draws (not mean) with high loglik ----------------------------
rm(list=setdiff(ls(), "config"))
load(paste0("../results/cpp/7param/variablephi-notemper/", config$modelName,"-",config$seed,"-7param-variablephi-notemper.rda"))
...
iSample <- as.integer(seq(1, dim(gpode$xsampled)[1], length=1000))
iSample <- iSample[order(-gpode$lglik[iSample])[1:100]]

phiNewInitSamples <- parallel::mclapply(iSample, function(i){
  xInit <- gpode$xsampled[i,,]
  phiNewInit <- gpds:::solveGpdsRcpp(
    yFull = xInit,
    odeModel = hes1logmodel,
...
## tempered with warm start and update phi from warm start average of posterior mean and maximum a posterior ----------------------------
rm(list=setdiff(ls(), "config"))
load(paste0("../results/cpp/7param/variablephi-notemper/", config$modelName,"-",config$seed,"-7param-variablephi-notemper.rda"))
...
phiNewInit <- gpds:::solveGpdsRcpp(
  ...
  odeModel = hes1logmodel,
  tvecFull = xsim$time,
  sigmaExogenous = pram.true$sigma/100,
  phiExogenous = matrix(numeric(0)),
  xInitExogenous = xInit,
  thetaInitExogenous = thetaInit,
  muExogenous = matrix(numeric(0)),
  ...
  useFixedSigma = config$useFixedSigma,
  verbose = TRUE)
phiPostMean <- phiNewInit$phi


i_map <- which.max(gpode$lglik)
xInit <- gpode$xsampled[i_map,,]
thetaInit <- gpode$theta[i_map,]
...
phiNewInit <- gpds:::solveGpdsRcpp(
  ...
  useFixedSigma = config$useFixedSigma,
  verbose = TRUE)
phiMAP <- phiNewInit$phi


xInit <- (apply(gpode$xsampled, 2:3, mean) + gpode$xsampled[i_map,,])/2
thetaInit <- (colMeans(gpode$theta) + gpode$theta[i_map,])/2
...
outDir <- "../results/cpp/7param/variablephi-temper-warmstart-updatephi-map-postmean/"
system(paste("mkdir -p", outDir))
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatephi-map-postmean.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatephi-map-postmean.rda"))


## tempered with warm start and update phi from warm start average of posterior mean and maximum a posterior (only for missing component) ----------------------------
rm(list=setdiff(ls(), "config"))
load(paste0("../results/cpp/7param/variablephi-notemper/", config$modelName,"-",config$seed,"-7param-variablephi-notemper.rda"))
...
phiExogenous[,3] <- phiNewInit$phi[,3]
...
outDir <- "../results/cpp/7param/variablephi-temper-warmstart-updatemissingphi-map-postmean/"
system(paste("mkdir -p", outDir))
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatemissingphi-map-postmean.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-7param-variablephi-temper-warmstart-updatemissingphi-map-postmean.rda"))
