...
phisigllik( rep(1,5), data.matrix(fn.sim[!is.nan(fn.sim[,1]),1:2]), r.nobs, signr.nobs, grad = TRUE, kerneltype)

fn <- function(par) -as.numeric(phisigllik( par, data.matrix(fn.sim[!is.nan(fn.sim[,1]),1:2]), 
                                            r.nobs, signr.nobs, grad = FALSE, kerneltype))
gr <- function(par) -attr(phisigllik( par, data.matrix(fn.sim[!is.nan(fn.sim[,1]),1:2]), 
                                       r.nobs, signr.nobs, grad = TRUE, kerneltype), "grad")
...
stepLow <- stepLow*0.02

burnin <- as.integer(n.iter*0.3)
n.iter.approx <- n.iter*0.00
for (t in 2:n.iter) {
  rstep <- runif(length(stepLow), stepLow, 2*stepLow)
  #rstep <- rstep / 10
  if(t < n.iter.approx){
