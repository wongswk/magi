...
if(!exists("config")){
  config <- list(
    ...
    seed = (as.integer(Sys.time())*104729+sample(1e9,1))%%1e9, # 1365546660, 
    npostplot = 50,
...
pram.true <- list(
  theta = c(0.022, 0.3, 0.031, 0.028, 0.5, 20),
...
modelODE <- function(t, state, parameters) {
  list(as.vector(gpds:::hes1logmodelODEfixg(parameters, t(state))))
}
...
hes1logmodel <- list(
  name="Hes1-log-fixg",
  fOde=gpds:::hes1logmodelODEfixg,
  fOdeDx=gpds:::hes1logmodelDxfixg,
  fOdeDtheta=gpds:::hes1logmodelDthetafixg,
...
gpode$fode <- sapply(1:length(gpode$lglik), function(t) 
  with(gpode, gpds:::hes1logmodelODEfixg(theta[t,], xsampled[t,,])), simplify = "array")
gpode$fode <- aperm(gpode$fode, c(3,1,2))

dotxtrue = gpds:::hes1logmodelODEfixg(pram.true$theta, data.matrix(xtrue[,-1]))
...
gpds:::plotPostSamplesFlex(
  paste0(outDir, config$modelName,"-",config$seed,"-fixg.pdf"), 
  xtrue, dotxtrue, xsim, gpode, pram.true, config, odemodel)

save.image(paste0(outDir, config$modelName,"-",config$seed,"-fixg.rda"))
