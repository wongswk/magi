...
## Timings:  run in shell first: for f in *.txt; do (cat "${f}"; echo) >> alltimes.csv; done
# timings <- read.table("alltimes.csv")
# apply(timings,2,mean)
# stdTimings <- apply(timings, 1, function(x) x/x[1])
# apply(stdTimings,1,mean)

# > apply(stdTimings,1,mean)
# V1       V2       V3 
# 1.000000 7.388911 5.077455 
# > apply(stdTimings,1,sd)
# V1        V2        V3 
# 0.0000000 1.2145035 0.8793506 
# > apply(timings,2,mean)
# V1       V2       V3 
