...
# Use this to run same seeds as previous Wenk and Dondel
...
### Optimize phi first using equally spaced intervals of 1.
samplesCpp <- gpds:::solveGpdsRcpp(
  #yFull = exoxInit,
  yFull = exoxInit[seq(1, length(xsim$time), by=2),],
  odeModel = ptransmodel,
  #tvecFull = xsim$time,
  tvecFull = xsim$time[seq(1, length(xsim$time), by=2)],
  ...
  #priorTemperatureLevel = 1,
  #priorTemperatureDeriv = 1,
  #priorTemperatureObs = 1/config$priorTemperature,
